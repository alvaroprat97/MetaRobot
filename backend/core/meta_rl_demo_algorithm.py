...
class MetaRLAlgorithm(metaclass=abc.ABCMeta):
    def __init__(
        ...
        print(f"Operating demonstrations from {self.demos.keys()} \n")
    ...
    def load_demos(self):
        ...
        """
        print("Operating on a single task family: Peg2D")
        for task_idx in self.train_tasks:
            demo_paths = self.demos['Peg2D'][task_idx]
            for path in demo_paths:
                path['context'] = [None]*path['rollout_length']
            num_demo_trans = sum([path['rollout_length'] for path in demo_paths])
            self.enc_replay_buffer.add_paths(task_idx, demo_paths)
            self.enc_replay_buffer.set_ground(task_idx, num_demo_trans)
            # Add it to replay buffer too for pretraining.
    ...
    def demo_criterion(self, path):
        if path['observations'].shape[0] < (self.enc_replay_buffer.task_buffers[self.task_idx].num_demo_trans() // 
                        self.enc_replay_buffer.task_buffers[self.task_idx].num_demos()):
            return True
        return False

    def collect_data(self, num_samples, resample_z_rate, update_posterior_rate, add_to_enc_buffer=True, actor = True, add_belief = False):
        '''
        ...
        if self.decoupled:
            if actor:
                self.agent['actor'].demo_clear_z()
            else:
                self.agent['xplor'].demo_clear_z()
        else:
            batch_size = min(
            ...
            demo_context = self.sample_context(self.task_idx, batch_size = batch_size)
        ...
        while num_transitions < num_samples:
            ...
            if add_to_enc_buffer:
                # Add to buffer only if...
                # Sparsify rewards
                for path in paths:
                    if self.demo_criterion(path):
                        # Sparsify rewards for the context encoder
                        path['rewards'] = np.array([[int(x) for x in path['terminals'].flatten()]])
                        self.enc_replay_buffer.add_path(self.task_idx, path)
            if update_posterior_rate != np.inf:
                num_demos = self.enc_replay_buffer.task_buffers[self.task_idx].num_demos()
                num_expert_trajs = self.enc_replay_buffer.task_buffers[self.task_idx].num_episode_starts()
                ...
                if num_expert_trajs > num_demos:
                    ...
                    extra_context = self.sample_context(self.task_idx, batch_size = 2*batch_size)
                    if not self.decoupled:
                        self.agent.infer_posterior(extra_context)
                    else:
                        if actor:
                            self.agent['actor'].infer_posterior(torch.cat([extra_context, demo_context]))
                        else:
                            self.agent['xplor'].infer_posterior(torch.cat([extra_context, demo_context]))
    ...
    def collect_paths(self, idx, epoch, run):
        ...
        # Reset the task environment & buffers for the randomly selected task
        self.env.reset_task(idx)
        demo_context = self.sample_context(self.task_idx, batch_size = self.embedding_batch_size)
