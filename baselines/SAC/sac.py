...
class SAC(object):
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        with torch.no_grad():
            next_state_action, next_state_log_pi, _ = self.policy.sample(next_state_batch, resample = False)
...
#         self.target_update_interval = target_update
#         self.automatic_entropy_tuning = bAutoEntropy
#
#         self.device = torch.device("cuda" if cuda else "cpu")
#         self.device = torch.device("cpu")
#         print("Working on CPU, GPU is too old")
...
#             # Target Entropy = −dim(A) (e.g. , -6 for HalfCheetah-v2) as given in the paper
#             if self.automatic_entropy_tuning is True:
#                 self.target_entropy = -torch.prod(torch.Tensor((3,3)).to(self.device)).item()
#                 self.log_alpha = torch.zeros(1, requires_grad=True, device=self.device)
#                 self.alpha_optim = Adam([self.log_alpha], lr=lr)
#
#             self.policy = GaussianPolicy(num_inputs, num_actions, hidden_size, action_range).to(self.device)
